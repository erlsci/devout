...
test_file_operations() ->
    ...
    ?assertEqual(ok, devout_fs:create_file(FilePath, <<"Hello, World!">>)),
    ...
    % Test file deletion
    ?assertEqual(ok, devout_fs:delete_file(FilePath)),
    ?assertNot(filelib:is_regular(ValidPath)),

    % Create a test file
    NestedPath = "subdir/nested_file.txt",
    ?assertEqual(ok, devout_fs:create_file(NestedPath, <<"Nested content">>)),
...
test_directory_operations() ->
    ...
    ?assertEqual(ok, devout_fs:create_directory(DirPath)),
    ...
    ?assertEqual(ok, devout_fs:create_directory(NestedDir)),
    ...
    EmptyDir = "empty_dir",
    ?assertEqual(ok, devout_fs:create_directory(EmptyDir)),
    ?assertEqual(ok, devout_fs:remove_directory(EmptyDir, false)),
...
test_security_features() ->
    ...
    case devout_fs:create_file(FilePath, LargeContent) of
        ok ->
            % If the fs layer doesn't enforce limits, that's ok for this test
            ?assert(true);
        {error, _} ->
...
test_error_handling() ->
    ...
    ?assertEqual({error, enoent}, devout_fs:delete_file("nonexistent.txt")),

    % Test deleting non-existent directory
    ?assertEqual({error, enoent}, devout_fs:remove_directory("nonexistent", false)),
